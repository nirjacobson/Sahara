#include "vulkancameradisplay.h"

Sahara::VulkanCameraDisplay::VulkanCameraDisplay(QVulkanWindow* window)
    : _vulkanWindow(window)
{
    initBuffers();
}

void Sahara::VulkanCameraDisplay::initBuffers()
{
    ...
    positionBuffer->write(positionData, numFloats * sizeof(float), 3);

    VulkanVertexBuffer* normalBuffer = new VulkanVertexBuffer(_vulkanWindow);
    normalBuffer->write(normalData, numFloats * sizeof(float), 3);
...
int Sahara::VulkanCameraDisplay::count() const
{
    return 15;
}
