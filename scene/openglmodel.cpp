...
#include "instance/openglinstancecontroller.h"
...
Sahara::OpenGLModel* Sahara::OpenGLModel::fromCollada(const QString& path)
{
  QCollada::Collada collada = QCollada::Collada::parse(path);
  Sahara::OpenGLModel* model = new OpenGLModel();
...
Sahara::ImageDict Sahara::OpenGLModel::parseColladaModelImages(const QCollada::Collada& collada, const QString& path)
{
  ...
  for (auto it = collada.images().begin(); it != collada.images().end(); it++) {
    ...
    Sahara::OpenGLImage* modelImage = new Sahara::OpenGLImage(id, QDir::cleanPath(path + QDir::separator() + image->initFrom()));
...
Sahara::MaterialDict Sahara::OpenGLModel::parseColladaModelMaterials(const QCollada::Collada& collada, const Sahara::ImageDict& images)
{
  ...
  for (auto it = collada.materials().begin(); it != collada.materials().end(); it++) {
    ...
    if ((effect = dynamic_cast<const QCollada::Effect*>(effectAsset))) {
      Sahara::OpenGLMaterial* modelMaterial;
      if (effect->sampler2D().has_value()) {
        QString imageId = effect->sampler2D()->source();
        Sahara::OpenGLImage* image = dynamic_cast<Sahara::OpenGLImage*>(images[imageId]);
        modelMaterial = new Sahara::OpenGLMaterial(
          id,
          material->name(),
...
Sahara::MeshDict Sahara::OpenGLModel::parseColladaModelGeometries(const QCollada::Collada& collada, Volume& volume)
{
  ...
  for (auto it = collada.geometries().begin(); it != collada.geometries().end(); it++) {
    ...
    Sahara::Mesh* modelMesh = new Sahara::OpenGLMesh(id);
...
Sahara::ControllerDict Sahara::OpenGLModel::parseColladaModelControllers(const QCollada::Collada& collada, MeshDict& meshes)
{
  ...
  if ((visualScene = dynamic_cast<const QCollada::VisualScene*>( collada.resolve(collada.scene()->instanceVisualScene().url()) ))) {
    ...
    visualScene->root()->depthFirst([&](const QCollada::Node& node) {
      ...
      if ((instanceGeometry = dynamic_cast<const QCollada::InstanceGeometry*>(node.item()))) {
        ...
        Sahara::OpenGLInstanceController* controllerInstance = new OpenGLInstanceController(armature, controllerInstanceMaterials, transformStack.top(), controller);
...
Sahara::Armature* Sahara::OpenGLModel::parseColladaArmatureNode(const QCollada::Node& rootNode)
{
...
Sahara::Joint* Sahara::OpenGLModel::parseColladaJointNode(const QCollada::Node& jointNode)
{
...
Sahara::AnimationDict Sahara::OpenGLModel::parseColladaModelAnimations(const QCollada::Collada& collada, Sahara::Armature& armature)
{
...
Sahara::AnimationClipDict Sahara::OpenGLModel::parseColladaModelAnimationClips(const QCollada::Collada& collada, const AnimationDict& animations)
{
...
Sahara::Animation* Sahara::OpenGLModel::parseColladaModelAnimation(const QString& id, const QCollada::Animation& animation, Sahara::Armature& armature)
{
