...
#include "instance/vulkaninstancecontroller.h"
...
Sahara::VulkanModel* Sahara::VulkanModel::fromCollada(VulkanRenderer* renderer, const QString& path)
{
  QCollada::Collada collada = QCollada::Collada::parse(path);
  Sahara::VulkanModel* model = new VulkanModel();
...
Sahara::ImageDict Sahara::VulkanModel::parseColladaModelImages(VulkanRenderer* renderer, const QCollada::Collada& collada, const QString& path)
{
  ...
  for (auto it = collada.images().begin(); it != collada.images().end(); it++) {
    ...
    Sahara::VulkanImage* modelImage = new Sahara::VulkanImage(renderer, id, QDir::cleanPath(path + QDir::separator() + image->initFrom()));
...
Sahara::MaterialDict Sahara::VulkanModel::parseColladaModelMaterials(VulkanRenderer* renderer, const QCollada::Collada& collada, const Sahara::ImageDict& images)
{
  ...
  for (auto it = collada.materials().begin(); it != collada.materials().end(); it++) {
    ...
    if ((effect = dynamic_cast<const QCollada::Effect*>(effectAsset))) {
      Sahara::VulkanMaterial* modelMaterial;
      if (effect->sampler2D().has_value()) {
        QString imageId = effect->sampler2D()->source();
        Sahara::VulkanImage* image = dynamic_cast<Sahara::VulkanImage*>(images[imageId]);
        modelMaterial = new Sahara::VulkanMaterial(
          renderer,
          id,
          material->name(),
...
Sahara::MeshDict Sahara::VulkanModel::parseColladaModelGeometries(QVulkanWindow* window, const QCollada::Collada& collada, Volume& volume)
{
  ...
  for (auto it = collada.geometries().begin(); it != collada.geometries().end(); it++) {
    ...
    Sahara::Mesh* modelMesh = new Sahara::VulkanMesh(window, id);
...
Sahara::ControllerDict Sahara::VulkanModel::parseColladaModelControllers(const QCollada::Collada& collada, MeshDict& meshes)
{
  ...
  if ((visualScene = dynamic_cast<const QCollada::VisualScene*>( collada.resolve(collada.scene()->instanceVisualScene().url()) ))) {
    ...
    visualScene->root()->depthFirst([&](const QCollada::Node& node) {
      ...
      if ((instanceGeometry = dynamic_cast<const QCollada::InstanceGeometry*>(node.item()))) {
        ...
        Sahara::InstanceController* controllerInstance = new VulkanInstanceController(renderer, armature, controllerInstanceMaterials, transformStack.top(), controller);
...
Sahara::Armature* Sahara::VulkanModel::parseColladaArmatureNode(const QCollada::Node& rootNode)
{
...
Sahara::Joint* Sahara::VulkanModel::parseColladaJointNode(const QCollada::Node& jointNode)
{
...
Sahara::AnimationDict Sahara::VulkanModel::parseColladaModelAnimations(const QCollada::Collada& collada, Sahara::Armature& armature)
{
...
Sahara::AnimationClipDict Sahara::VulkanModel::parseColladaModelAnimationClips(const QCollada::Collada& collada, const AnimationDict& animations)
{
...
Sahara::Animation* Sahara::VulkanModel::parseColladaModelAnimation(const QString& id, const QCollada::Animation& animation, Sahara::Armature& armature)
{
