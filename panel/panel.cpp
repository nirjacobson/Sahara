...
Sahara::Panel::Panel(Renderer *renderer, int width, int height)
    : _renderer(renderer)
    , _size(width, height)
    , _position(0, 0)
...
void Sahara::Panel::generateVertexBuffers()
{
    ...
    VertexBuffer* vertexBuffer = new VertexBuffer(_renderer->window());
    ...
    vertexBuffer = new VertexBuffer(_renderer->window());
